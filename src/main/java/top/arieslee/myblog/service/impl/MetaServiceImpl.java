...
//    private IRelationshipService relationshipService;
    ...
    public MetaDto getMeta(String type, String name) {
        if (StringUtils.isNotBlank(type) && StringUtils.isNotBlank(name)) {
            return metaVoDao.selectDtoByNameAndType(name, type);
        }
        return null;
    }
    ...
    public void saveMeta(String type, String name, Integer mid) {
        if (StringUtils.isNotBlank(type) && StringUtils.isNotBlank(name)) {
            ...
            List<MetaVo> metaVos = metaVoDao.selectByExample(metaVoExample);
            MetaVo metas;
            if (metaVos.size() != 0) {
                ...
                metas.setName(name);
                if (null != mid) {
                    MetaVo original = metaVoDao.selectByPrimaryKey(mid);
                    metas.setMid(mid);
                    metaVoDao.updateByPrimaryKeySelective(metas);
//                    更新原有文章的categories
                    contentService.updateCategory(original.getName(), name);
                } else {
                    metas.setType(type);
                    metaVoDao.insertSelective(metas);
                }
            }
        }
    }
    ...
    public List<MetaDto> getMetaList(String type, String orderby, int limit) {
        if (StringUtils.isNotBlank(type)) {
            ...
            return metaVoDao.selectFromSql(paraMap);
        }
        return null;
    }
...
//        }
    ...
    public void delete(int mid) {
        MetaVo metas = metaVoDao.selectByPrimaryKey(mid);
        if (null != metas) {
            String type = metas.getType();
            String name = metas.getName();

            metaVoDao.deleteByPrimaryKey(mid);
