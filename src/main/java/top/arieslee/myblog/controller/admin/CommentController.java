...
public class CommentController extends BaseController {
    ...
    @ResponseBody
    public ResponseDto delete(@RequestParam Integer coid) {
        try {
            CommentVo comments = commentsService.getCommentById(coid);
            if (null == comments) {
                return ResponseDto.fail("不存在该评论");
            }
    ...
    @ResponseBody
    public ResponseDto delete(@RequestParam Integer coid, @RequestParam String status) {
        try {
            CommentVo comments = commentsService.getCommentById(coid);
            if (comments != null) {
